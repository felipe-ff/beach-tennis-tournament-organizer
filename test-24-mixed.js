// Teste balanceado para 24 jogadores em duplas mistas (12 homens + 12 mulheres)
...
function test24PlayersBalanced() {
  console.log("=== TESTE BALANCEADO DUPLAS MISTAS - 24 JOGADORES ===");
  console.log("12 Homens (1-12) + 12 Mulheres (13-24)\n");
  
  // Estrutura original para 24 jogadores
  const originalStructure = {
    rounds: [
      ...
      [
        ...
        { game: 23, team1: [5, 7], team2: [6, 8] },
        { game: 24, team1: [17, 19], team2: [18, 20] }
      ]
    ]
  };
  
  const playerCount = 24;
  const halfCount = playerCount / 2; // 12
  const menNumbers = Array.from({ length: halfCount }, (_, i) => i + 1);      // [1,2,3...12]
  const womenNumbers = Array.from({ length: halfCount }, (_, i) => halfCount + i + 1); // [13,14,15...24]
    ...
    rounds: originalStructure.rounds.map((round, roundIndex) => 
      round.map((game) => {
        ...
        if (!team1Pair || !team2Pair) {
          console.warn(`⚠️ Não foi possível encontrar pares balanceados para o jogo ${game.game} (rodada ${roundIndex + 1})`);
  ...
  console.log(`${validPairs === 48 ? '✅' : '❌'} Duplas válidas (1H+1M): ${validPairs}/48`);
  ...
  // Determinar sucesso
  const success = !hasRepeatedPairs && validPairs === 48 && invalidPairs === 0 && maxGames - minGames <= 1;
  console.log(`\n${success ? '🎾 EXCELENTE' : '⚠️ ACEITÁVEL'}: Implementação para 24 jogadores ${success ? 'está funcionando perfeitamente!' : 'precisa de ajustes'}`);
  
  return success;
}

test24PlayersBalanced();
