// Teste balanceado para 16 jogadores em duplas mistas (8 homens + 8 mulheres)
...
function test16PlayersBalanced() {
  console.log("=== TESTE BALANCEADO DUPLAS MISTAS - 16 JOGADORES ===");
  console.log("8 Homens (1-8) + 8 Mulheres (9-16)\n");
  
  // Estrutura original para 16 jogadores
  const originalStructure = {
    rounds: [
      ...
      // Rodada 3
      [
        { game: 9, team1: [1, 9], team2: [5, 3] },
        { game: 10, team1: [2, 10], team2: [6, 4] },
        { game: 11, team1: [7, 11], team2: [13, 15] },
        { game: 12, team1: [8, 12], team2: [14, 16] }
      ],
      // Rodada 4
      [
        { game: 13, team1: [1, 13], team2: [9, 7] },
        { game: 14, team1: [2, 14], team2: [10, 8] },
        { game: 15, team1: [3, 11], team2: [5, 15] },
        { game: 16, team1: [4, 12], team2: [6, 16] }
      ]
    ]
  };
  
  const playerCount = 16;
  const halfCount = playerCount / 2; // 8
  const menNumbers = Array.from({ length: halfCount }, (_, i) => i + 1);      // [1,2,3,4,5,6,7,8]
  const womenNumbers = Array.from({ length: halfCount }, (_, i) => halfCount + i + 1); // [9,10,11,12,13,14,15,16]
  ...
  console.log(`Total de pares possíveis (H+M): ${shuffledMen.length * shuffledWomen.length}`);
  ...
  console.log(`${validPairs === 32 ? '✅' : '❌'} Duplas válidas (1H+1M): ${validPairs}/32`);
  console.log(`${invalidPairs === 0 ? '✅' : '❌'} Duplas inválidas: ${invalidPairs}`);
  console.log(`${perfectBalance ? '✅' : maxGames - minGames <= 1 ? '⚠️' : '❌'} Balanceamento: ${perfectBalance ? 'PERFEITO' : maxGames - minGames <= 1 ? 'MUITO BOM' : 'PRECISA AJUSTE'} (diferença: ${maxGames - minGames})`);
  console.log(`📊 Jogos por pessoa: min=${minGames}, max=${maxGames}, média=${(gameCounts.reduce((a, b) => a + b, 0) / gameCounts.length).toFixed(1)}`);
  console.log(`🎯 Utilização de pares: ${partnerships.size}/${shuffledMen.length * shuffledWomen.length} (${((partnerships.size / (shuffledMen.length * shuffledWomen.length)) * 100).toFixed(1)}%)`);
  
  // Determinar sucesso
  const success = !hasRepeatedPairs && validPairs === 32 && invalidPairs === 0 && maxGames - minGames <= 1;
  console.log(`\n${success ? '🎾 EXCELENTE' : '⚠️ ACEITÁVEL'}: Implementação para 16 jogadores ${success ? 'está funcionando perfeitamente!' : 'precisa de ajustes'}`);
  
  return success;
}

test16PlayersBalanced();
